...
from core.profile_service import (
    randomize_profile, change_profile_name, change_profile_bio,
...
class ActionsTab(QWidget):
    def __init__(self, account_manager: AccountManager, task_executor: TaskExecutor,
                 log_widget: LogWidget, get_thread_settings=None, get_delay=None,
                 parent=None):
    ...
    def _init_ui(self):
        ...
        self.cb_background = QCheckBox("Set random background (from inventory)")
        self.cb_full_profile = QCheckBox("Randomize all profile elements")
        ...
        profile_layout.addWidget(self.cb_full_profile, 2, 0, 1, 2)
    ...
    def _start(self):
        ...
        # Shared state for unique groups across accounts
        used_groups = set()

        # Read min/max ranges from spinners
        ...
        if self.cb_background.isChecked():
            actions.append(("Random Background", change_profile_background))
        if self.cb_full_profile.isChecked():
            actions.append(("Full Profile Randomize", randomize_profile))
        ...
        self.progress.setValue(0)
        self.progress.setMaximum(len(selected) * len(actions))

        self._completed = 0

        # Connect signals only once to avoid duplicate handlers
